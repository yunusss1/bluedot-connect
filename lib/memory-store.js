...
// Initialize from localStorage if available - TÜM DATA TİPLERİ
if (isBrowser) {
  const savedDrivers = loadFromLocalStorage('drivers');
  const savedCampaigns = loadFromLocalStorage('campaigns');
  const savedRecordings = loadFromLocalStorage('recordings');
  const savedTranscripts = loadFromLocalStorage('transcripts');
  
  if (savedDrivers) memoryStore.drivers = savedDrivers;
  if (savedCampaigns) memoryStore.campaigns = savedCampaigns;
  if (savedRecordings) memoryStore.recordings = savedRecordings;
  if (savedTranscripts) memoryStore.transcripts = savedTranscripts;
}
